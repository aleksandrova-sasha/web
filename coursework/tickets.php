<?php
	...
	*/
	...
	$param_keys=array('user_id', 'event_id', 'num'); // названия полей в таблице БД
	$param_str=array('Пользователь', 'Сеанс', 'Номер места'); // названия столбцов в таблице для отображения
...
<?php
	...
	$query="
		SELECT
			$buf, `events`.dt AS 'Дата'
			$admin_delete
			$admin_edit
		FROM
			`$table`, `users`, `events`
		WHERE 1
			AND `users`.`id`=`tickets`.`user_id`
			AND (
